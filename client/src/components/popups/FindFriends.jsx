      ...
      {showFriendRequests ? friends.map((friend) => (
        <div className="d-flex m-2 mx-auto" style={{ maxWidth: '800px' }} key={friend.id}>
          ...
          <button
            ...
            onClick={(e) => handleFriendDelete(friend.id, e)}
            type="button"
          >
            Remove Friend
          </button>
      ...
      {showFriendRequests ? friendRequests.map((friend) => (
        <div className="d-flex m-2 mx-auto" style={{ maxWidth: '800px' }} key={friend.id}>
          ...
          <button
            disabled={loading}
            className="btn btn-primary"
            onClick={(e) => handleFriendRequestAccept(friend.id, e)}
            type="button"
          >
      ...
      <div className="mx-auto" style={{ maxWidth: '800px' }}>
        {friendResults.map((friend) => (
          <div className="d-flex m-2" key={friend.id}>
            ...
            <button disabled={loading} className="btn btn-primary" type="button" onClick={(e) => handleFriendRequest(friend.id, e)}>Add Friend</button>
          </div>
        ))}
      </div>
    </div>

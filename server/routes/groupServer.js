...
router.post("/find", verify, async (req, res) => {
    if (req.body.type === "find" && req.body.userId){
        ...
        if (_groupServers.length > 0){
            ...
            promise.then(() => {
                //Return the group servers to client as well as the textChannels and invites if they were requested
                res.status(200).json({
                    success: true, 
                    message: "Success", 
...
router.post("/verify", verify, async (req, res) => {
    if (req.body.type === "verify" && 
        ...
        try{
            const groupServer = await GroupServer.find({_id: req.body.groupServerId, users: req.body.userId});
            const textChannel = await TextChannel.findById(req.body.textChannelId);
    ...
    if (req.body.type === "create-invite" 
        ...
        if (groupServer !== null){
            ...
            if (!(invite instanceof Error) && invite !== null) 
                ...
                res.status(500).json({success: false, message: "Failed", err: invite});
        }
    ...
    if (req.body.type === "delete-channel" && 
        ...
        if (groupServer !== null){
            ...
            if (authorized){
                ...
                await GroupServer.findByIdAndUpdate(req.body.groupServerId, {$pull : {textChannels : req.body.textChannelId}});
